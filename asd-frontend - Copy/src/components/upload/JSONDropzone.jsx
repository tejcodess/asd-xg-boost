...
const JSONDropzone = ({ onFileSelect, isLoading }) => {
  ...
  const validateAndSetFile = (file) => {
    ...
    // Check file size (limit to 50MB for large datasets)
      ...
      >
        {!selectedFile ? (
          <>
            ...
            <p className="text-xs text-slate-500">JSON file with MongoDB change stream or CSV data field</p>
          </>
        ) : (
          <div className="flex flex-col items-center p-6 text-center">
            <div className="relative">
              <FileJson className="w-16 h-16 text-clinical-blue mb-4" />
              {!isLoading && (
                <button 
                  onClick={clearFile}
