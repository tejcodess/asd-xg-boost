...
const VideoDropzone = ({ onFileSelect, isLoading }) => {
  ...
  const validateAndSetFile = (file) => {
    ...
    // 1. Check MIME types (Added 'video/avi' just in case)
    const validTypes = ['video/mp4', 'video/quicktime', 'video/x-msvideo', 'video/avi'];
    const isValidType = validTypes.includes(file.type);

    // 2. Check File Extensions (Browsers often fail to assign a MIME type to AVI files)
    const fileName = file.name.toLowerCase();
    const isValidExtension = fileName.endsWith('.mp4') || fileName.endsWith('.mov') || fileName.endsWith('.avi');

    if (!isValidType && !isValidExtension) {
      setError('Please upload a valid MP4, MOV, or AVI file.');
      return;
    }
      ...
      >
        {!selectedFile ? (
          <>
            ...
            <p className="text-xs text-slate-500">MP4, AVI, or MOV (Max 2 minutes)</p>
          </>
        ) : (
          <div className="flex flex-col items-center p-6 text-center">
            <div className="relative">
              <FileVideo className="w-16 h-16 text-clinical-blue mb-4" />
              {!isLoading && (
                <button 
                  onClick={clearFile}
